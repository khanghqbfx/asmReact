...
const Login = () => {
  ...
  const submitFormHandler = (event) => {
    ...
    //lấy danh sach người dùng từ Local
    const data = JSON.parse(localStorage.getItem("user_accounts"));

    // tìm và kiểm tra người dùng có tồn tại   và thông tin đăng nhập
    if (data === null) {
    ...
    if (curData !== null && curData.password === formData.password && !hasError) {
    ...
    if(curData && curData.islogin === true) {
      data[index] = { ...data[index], isLogin: true };
      localStorage.setItem("user_accounts", JSON.stringify(data));
      dispatch(loginActions.login(curData));
      navigate("/");
